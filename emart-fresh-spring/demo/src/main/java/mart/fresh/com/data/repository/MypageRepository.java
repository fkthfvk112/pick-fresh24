...
public interface MypageRepository extends JpaRepository<Member, String>{
	...
	ApplyManager getMemberAndIsAppliedByMemberId(@Param("memberId") String memberId);
	 ...
	 @Query("UPDATE Member m "
	      + "SET m.memberPw = :newPw "
	      + "WHERE m.memberId = :memberId AND m.memberPw = :memberPw")
	 int changePassword(@Param("memberId") String memberId, @Param("memberPw") String memberPw, @Param("newPw") String newPw );
	 
	 
	 @Query("SELECT COUNT(m) "
			+ "FROM Member m "
			+ "WHERE m.memberEmail = :newEmail")
